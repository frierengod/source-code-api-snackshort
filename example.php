<?php
/**
 * Contoh Script Bridge API Snackshort via StreamAPI.web.id
 * ----------------------------------------------------
...
// Fungsi bantuan buat manggil endpoint Snackshort di sistem Bridge
function callSnackshortAPI($urlTujuan, $token) {
...
// UJI COBA KODE
// -----------------------------------------------------------------
$baseUrl = "https://streamapi.web.id/p/snackshort/api/v1";

// Contoh 1: Fitur Cari Film Snackshort
echo "\n=====================[ 1. Pencarian Snackshort ]==================\n";
$kataKunci = urlencode("cinta");
$halaman = 1;
$urlCari = "{$baseUrl}/search/{$kataKunci}/{$halaman}?lang=in";

$hasilCari = callSnackshortAPI($urlCari, $API_TOKEN);
if ($hasilCari) {
...
$hasilDetail = callSnackshortAPI($urlDetail, $API_TOKEN);
if ($hasilDetail) {
...
$hasilNonton = callSnackshortAPI($urlNonton, $API_TOKEN);
if ($hasilNonton) {
...
echo "▶▶▶ https://streamapi.web.id/docs/snackshort ◀◀◀\n\n";
